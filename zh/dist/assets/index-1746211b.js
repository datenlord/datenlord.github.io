const t="/datenlord.github.io/zh/assets/cover-6865ae02.png",l="/datenlord.github.io/zh/assets/image1-ad36726d.png",i="/datenlord.github.io/zh/assets/image2-b4bfa07d.png",o="/datenlord.github.io/zh/assets/image3-650e4585.png",p=[t,l,i,o],e={label:"分布式状态机共识协议 Copilot",description:"现有的分布式状态机共识协议，不论其是何种流派，何种实现，都在模型中将节点状态抽象为仅有“在线”与“下线”两种情况。然而在实践上，节点状态并非只有这两种互斥的状态——节点可能因配置错误，节点侧网络问题，部分硬件出现问题，垃圾回收事件，以及其它很多原因而导致响应速度变慢（slowdown）。由于现有的共识算法没有很好地处理这一状态，系统内一个节点 slowdown 可能导致整个系统的响应速度变慢。Copilot 协议旨在解决这一问题：它可以在任何情况下可以容忍系统中 1 个节点发生 slowdown（1-slowdown-tolerant）。",cover:"./cover.png",location:"中国香港",tags:["Xline"],date:"2022-12-15",title:"Distributed state machine consensus protocol Copilot"},n=[{label:"前言",level:2},{label:"定义 slowdown",level:3},{label:"为什么现有的共识协议无法容忍 slowdown",level:3},{label:"Copilot 如何处理 slowdown",level:3},{label:"设计",level:2},{label:"模型",level:3},{label:"排序",level:3},{label:"执行",level:3},{label:"Fast Takeover",level:3},{label:"额外设计",level:3},{label:"Copilot 能达到 1-slowdown-tolerant 的原因",level:3},{label:"优化",level:2},{label:"Ping-Pong Batching",level:3},{label:"Null Dependency Elimination",level:3},{label:"总结",level:2}],a=`<p><img src="${t}" alt="封面"></p>
<p><strong>论文原文：Tolerating Slowdowns in Replicated State Machines using Copilots</strong><br>
以下内容是对这篇论文的<strong>阅读总结</strong>，以及<strong>部分重要章节（§3 Design、§5 Optimizations）的翻译</strong>。</p>
<h2 id="前言">前言</h2>
<p>现有的分布式状态机共识协议，不论其是何种流派，何种实现，都在模型中将节点状态抽象为仅有“在线”与“下线”两种情况。然而在实践上，节点状态并非只有这两种互斥的状态——节点可能因配置错误，节点侧网络问题，部分硬件出现问题，垃圾回收事件，以及其它很多原因而导致响应速度变慢（slowdown）。由于现有的共识算法没有很好地处理这一状态，系统内一个节点 slowdown 可能导致整个系统的响应速度变慢。Copilot 协议旨在解决这一问题：它可以在任何情况下可以容忍系统中 1 个节点发生 slowdown（1-slowdown-tolerant）。</p>
<h3 id="定义-slowdown">定义 slowdown</h3>
<p>要想解决这一问题，首先要对 slowdown 进行明确的定义。</p>
<p>首先要定义一个节点的处理速度。一个节点从收到一条消息开始，到其处理这条消息完毕，将回复送出为止，这段时间的长短就是一个节点的处理速度。这段时间不包含消息在网络链路上的传输时间，仅包含消息在节点本机处理所需的时间。假设一个节点处理一条消息的典型时间为 1ms，而设置超时阈值 t = 10ms，那么如果该节点处理一条消息花费了大于 10ms，就说明该节点出现了 slowdown。出错（failed）的节点一定是 slowdown 的，但 slowdown 的节点不一定 failed——它只是响应速度变慢，而不是停止响应。</p>
<p>下面定义 <strong>s-slowdown-tolerance</strong>。当一个系统中有 s 个节点发生 slowdown 时，系统仍能正常地提供服务，那么这个系统就是 s-slowdown-tolerance。假设一个分布式状态机系统有节点集 {r1, ..., rs, ..., rn}，且根据目前的 “slowdown” 定义排序，其中 {r1, ..., rs} 是最慢的节点。定义当前整个系统的处理速度为 T，并定义 T' 为将系统中节点 {r1, ..., rs} 全部替换为 rs + 1 时整个系统的处理速度。如果 T 与 T' 的差值接近 0，那么该系统就是 s-slowdown-tolerance。换句话讲，就是系统中出现 s 个节点 slowdown 不会影响整个系统的处理速度。</p>
<h3 id="为什么现有的共识协议无法容忍-slowdown">为什么现有的共识协议无法容忍 slowdown</h3>
...
<h3 id="copilot-如何处理-slowdown">Copilot 如何处理 slowdown</h3>
<p>根据上一节，只要保证 在处理一条客户端指令过程中的任意时间点，都有大于等于 s + 1 条路径处理该条客户端指令，那么该系统就是 s-slowdown-tolerant。</p>
<p>Copilot 容忍一个节点 slowdown，因此需要在处理客户端指令时有两条路径。</p>
<p><img src="${o}" alt="图片"></p>
<h2 id="设计">设计</h2>
<p><strong>本节内容是对论文 §3 Design 的翻译。</strong>
Copilot 的设计核心是同时使用两个 leader 互为冗余地处理每一条 client 指令。两个 leader 为 pilot (P) 与 copilot (P')。</p>
<h3 id="模型">模型</h3>
<ul>
...
<h4>节点回复 FastAccept</h4>
<p>当一节点收到 FastAccept 消息时，它会检查收到的指令初始依赖是否与其之前已 Accept 的其它 entry 冲突。若不冲突，节点会回复此 FastAcceptOk。若冲突，节点会回复其认为正确的依赖顺序。</p>
<p>compability check：<br>
对于两个依赖，如果其中至少有一个的 entry 位于另一个的 entry 之后，则两依赖不冲突。在上图 a 中表示了冲突与未冲突的依赖：依赖于 P.1 的 P'.1 与依赖于 P'.1 的 P.2 没有冲突，因为 P.2 位于 P'.1 之后. 依赖于 P.2 的 P'.3 与依赖于 P'.2 的 P.3 冲突，因为它们都不位于对方 entry 之后。由于冲突的依赖可能导致指令执行顺序不一致，所以必须避免，如有节点会先执行 P.3 再执行 P'.3，有节点会反之。</p>
...
<h4>Pilot 尝试通过 fast path 来 commit 该指令</h4>
...
<p>不论哪种原因，pilot 都需要等待有至少 f + 1 个节点回复，才能进入下一步。</p>
<h4>Pilot 在 Accept 阶段最终确定依赖</h4>
...
<h3 id="fast-takeover">Fast Takeover</h3>
<p>为了达到执行顺序一致，有时一 pilot 需要等待另一 pilot 的 commit 消息。正常工作的 pilot 等待另一 slowdown 的 pilot 会消耗大量时间，因此需要 Fast Takeover 机制。<br>
每个节点 log 中的每一个 entry 都会附有一 ballot number。与 Paxos 的 proposal number 类似，Copilot 协议过程中的每条消息都会带有 ballot number。通过这些序列号可以让 pilot 安全地进行与 Paxos 类似的所有权竞争的 Fast Takeover。</p>
...
<h4>entry 内容的恢复过程</h4>
<p>Fast Takeover 与 view-change 都使用了本过程，以正确恢复任意可被 commit 并执行的 entry。本过程十分复杂，细节可参阅 Technical Report TR-004-20。</p>
...
<h3 id="额外设计">额外设计</h3>
<p>本节未提到的 Copilot 设计都与其它分布式状态机的设计一致。</p>
<p>保证指令仅被执行一次是通过检查 cliid 与 cid 实现的。记录一条指令已被执行一次的 cliid 与 cid 容器会在第二次遇到相同指令时清空。</p>
<p>对于结果非确定的指令（如涉及随机数生成，每次执行结果都不同），可以由两 pilot 完成非确定部分（随机数生成），此时该指令的执行结果变为可确定的值，再进行排序与执行等操作。两 pilot 在完成非确定部分时可能不同，所以最终的指令可能的执行结果会有两个版本，但执行的去重机制可以保证只有一个结果有效。</p>
...
<h3 id="copilot-能达到-1-slowdown-tolerant-的原因">Copilot 能达到 1-slowdown-tolerant 的原因</h3>
<p>Copilot 通过保证任何客户端指令在任意时间节点都在两条不同的处理路径上。两条路径的处理速度不同，处理速度快的路径得到的结果必定先返回客户端。</p>
<p>当两 pilot 都为正常状态时，就算有最多 f 个其它节点 slowdown 时，1-slowdown-tolerant 仍成立，因为在 regular path 上 commit 只需要 f + 1 个节点回复。<br>
当一 pilot 发生 slowdown 或出错时，另一个正常的 pilot 仍可以正常 commit 其 entry，如果有对 slowdown pilot 指令的依赖，正常 pilot 会执行 fast takeover 解决这个问题。在 slowdown 发生后，正常 pilot 会通过 fast takeover 迅速的解决对 slowdown pilot 指令的依赖，之后即可按正常流程继续处理。此时该分布式状态机的总体执行速度取决于此正常的 pilot，所以 1-slowdown-tolerant 成立。</p>
<h2 id="优化">优化</h2>
<p><strong>本节内容是对论文 §5 Optimizations 的翻译。</strong></p>
<h3 id="ping-pong-batching">Ping-Pong Batching</h3>
...
<p>在 Ping-Pong Batching 过程中，各 pilot 会打包（batching）在一定时间段内到达的指令。在此期间该 pilot 每收到一条指令都会将其放入自己的下一个 entry 中，因此打包后的指令会是一组连续的 entry。当该 pilot 收到另一 pilot 的 FastAccept 消息，或到达设定的打包最长时间段（ping-pong-wait timeout），就会结束当前的打包操作。</p>
...
<p>当有一 pilot 出现 slowdown 时，另一 pilot 会在达到 ping-pong-wait timeout 后结束打包操作。这避免了因一个 pilot 出现 slowdown 导致整个系统 slowdown。</p>
<h3 id="null-dependency-elimination">Null Dependency Elimination</h3>

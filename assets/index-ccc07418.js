const e="/assets/image1-5f980d41.png",t="/assets/image2-9738d1ab.png",a="/assets/image3-61af2ce5.png",r="/assets/image4-baa52a0c.png",o=[e,t,a,r],i={label:"GPUDirect RDMA 的演进与实现",description:"为了解决数据传输效率低下的问题，实现网络与 GPU 之间真正的高速、低延迟直连通道，GPUDirect RDMA 技术应运而生。该技术的核心目标是使 RDMA 网卡能够直接、安全地访问本地或远程 GPU 的显存，完全绕过主机 CPU 和系统内存。通过建立这样一个直接的数据通路，GPUDirect RDMA 能够最大程度地发挥 RDMA 网卡和 GPU 并行计算的潜力，极大地减少数据移动开销，提升整体应用的性能。",location:"中国香港",cover:"./image1.png",author:["陈添"],date:"2025-04-10",title:"The Evolution and Implementation of GPUDirect RDMA"},n=[{label:"引言",level:2},{label:"GPUDirect RDMA 的诞生",level:2},{label:"内核标准化 P2P 内存访问的探索：Peer Memory Client",level:2},{label:"AMD 的实践：基于 Peer Memory Client 的 ROCnRDMA",level:2},{label:"内核的选择：基于 dma-buf 的 P2P DMA",level:2},{label:"总结",level:2}],p=`<p><img src="${e}" alt="图片"></p>
<h2 id="引言">引言</h2>
...
</section>`;export{o as assetURLs,p as default,i as metadata,n as toc};
